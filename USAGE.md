...
the frame keeps the full 95% of the picture. Before Step 4 renders anything, you must Add both
...
draw it resized (aspect preserved, never cropped, filtered, rounded, backgrounded, or overprinted) inside
a fixed 305×305 square flush in the frame's top corners (corners rounded like the frame),
and the QR picture keeps its own pixels with no white card. Step 4 refuses to
