...
- **Two required page pictures** - Add a 1:1 image (left slot) and a QR scan image (right slot) in the Create Video step before rendering; both are copied byte-identically into the job folder and Step 4 refuses to run without them. Pages draw each picture resized only (aspect preserved, never cropped/filtered/overprinted) inside a 201×201 square centered on the body band, with a white card and quiet zone behind the QR. A full-frame preview renders the exact final page with the same pictures before any video is created; changing either picture re-renders the pages.
